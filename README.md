...
The server also serves a web build of the app (installable PWA) same-origin
with the API - visit the server's own address in a browser (e.g.
`http://SERVER_IP:8090`) and "Add to Home Screen" / "Install". No LAN
discovery needed on web since it's already same-origin.

The web build isn't auto-generated by a plain `go build`; run this first
(the release binaries from CI already have it embedded):

```sh
cd apps/mobile && npx expo export -p web --output-dir ../../server/web/dist
```
